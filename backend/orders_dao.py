...
def delete_order_by_order_number(connection, order_number):
    cursor = connection.cursor()

    try:
        # First, delete related rows in order_details table
        cursor.execute("DELETE FROM order_details WHERE order_id = %s", (order_number,))
        # Then, delete the order in orders table
        cursor.execute("DELETE FROM orders WHERE order_id = %s", (order_number,))
        connection.commit()
        return True
